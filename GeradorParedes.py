...
def AutoGerenciamentoParedes(tabela_ambientes):
    lados_fundamentais_poligono = LadosFundamentais(tabela_ambientes)
    paredes = []
    for parede in lados_fundamentais_poligono:
        vertices = parede[1]
...
def IdentificarAmbientesParedes(paredes, lados_fundamentais_poligono):
    ...
    for parede in paredes:
        ...
        angulo = Angulo(parede)
        tabela_paredes.append([ambientes, parede, andar_count, angulo])
    return tabela_paredes
...
def LadosFundamentais(tabela_ambientes):
    ...
    for item in tabela_ambientes:
        ...
        buracos = item[6]
        if(buracos!=[]):
            for vertices_internos in buracos:
                for i_vertice in range(len(vertices_internos)):
                    p1 = vertices_internos[i_vertice]
                    p2 = vertices_internos[(i_vertice + 1) % len(vertices_internos)]
                    parede = [nome_ambiente,[p1,p2]]
                    lados_polig.append(parede)
...
def Angulo(linha):
    dx = linha[1][0] - linha[0][0]
    dy = linha[1][1] - linha[0][1]

    inclinaçao = math.atan2(dy, dx)
    angulo_em_radianos = inclinaçao
    angulo_em_graus = math.degrees(angulo_em_radianos)
    if angulo_em_graus  < 0:
        angulo_em_graus  += 360
    return angulo_em_graus
...
def exemplo3():
    ...
    ambiente1 = ["ambiente1", 0,0,0,0,poligono1]
    ambiente2 = ["ambiente2", 0,0,0,0,poligono2]
    ambiente3 = ["ambiente3", 0,0,0,0,poligono3]
    tabela_ambientes = [ambiente1, ambiente2, ambiente3]



    resultado = AutoGerenciamentoParedes(tabela_ambientes)
    print("entrada:")
...
def Iniciar(tabela_ambientes, _andar_count):
    global andar_count
    andar_count = _andar_count
    tabela_paredes = AutoGerenciamentoParedes(tabela_ambientes) 
    return tabela_paredes